...
export function QuickViewButton({ slug }: { slug: string }) {
  ...
  return (
    <>
      ...
      >
        <Eye className="inline h-3 w-3 mr-1.5 -mt-0.5" /> Xem nhanh
      </button>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl p-0 grid md:grid-cols-2 gap-0 overflow-hidden">
          <DialogTitle className="sr-only">{product?.name ?? 'Sản phẩm'}</DialogTitle>
          {loading || !product ? (
            <div className="md:col-span-2 p-12 text-center text-muted-foreground">Đang tải…</div>
          ) : (
            <>
              ...
              <div className="p-8 flex flex-col">
                ...
                <div className="mt-4">
                  <p className="text-xs uppercase tracking-widest mb-2">Kích cỡ</p>
                  <div className="flex flex-wrap gap-2">
                    {product.sizes?.map((s: string) => (
                      <button key={s} onClick={() => setSize(s)}
                        className={`h-9 min-w-11 px-2 grid place-items-center border text-sm ${size === s ? 'border-indigo-900 bg-indigo-900 text-denim-ecru' : 'border-border'}`}>
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                <div className="mt-auto pt-6 space-y-2">
                  <Button onClick={onAdd} size="lg" className="w-full">Thêm vào giỏ</Button>
                  <div className="grid grid-cols-2 gap-2">
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/shop/${product.slug}`}>Xem chi tiết</Link>
                    </Button>
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/try-on?productId=${product.id}`}><Sparkles className="h-3 w-3" /> Thử AI</Link>
                    </Button>
                  </div>
                </div>
              </div>
