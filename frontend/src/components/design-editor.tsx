...
export function DesignEditor() {
  ...
  const [title, setTitle] = useState('Thiết kế của tôi');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      canvas = new fabric.Canvas(canvasRef.current, {
        width: 600, height: 720, backgroundColor: color, preserveObjectStacking: true,
      });
  ...
  async function addText() {
    ...
    const text = new fabric.IText('TEXT CỦA BẠN', {
      left: 200, top: 320, fill: '#f5efe6', fontFamily: 'serif', fontSize: 36,
    });
  ...
  function removeActive() {
    ...
    const obj = c?.getActiveObject(); if (obj) { c.remove(obj); c.discardActiveObject().renderAll(); }
  }
  ...
  async function save(submit = false) {
    ...
    try {
      ...
      toast.success(submit ? 'Đã gửi cho designer review' : 'Đã lưu bản nháp');
    } catch (e: any) {
  ...
  return (
    <div className="grid lg:grid-cols-[280px_1fr] gap-8">
      <aside className="space-y-6">
        <div>
          <Label className="mb-1.5 block">Tên thiết kế</Label>
          <Input value={title} onChange={(e) => setTitle(e.target.value)} />
        </div>
        <div>
          <Label className="mb-1.5 block flex items-center gap-2"><Palette className="h-3 w-3" /> Màu nền</Label>
          <div className="flex gap-2 items-center">
            <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="h-9 w-12" />
            <Button variant="outline" size="sm" onClick={applyBg}>Áp dụng</Button>
          </div>
        </div>
        <div className="space-y-2">
          <p className="text-xs uppercase tracking-widest">Thêm element</p>
        ...
        <div>
          <p className="text-xs uppercase tracking-widest mb-2">Thư viện patch</p>
        ...
        <Button variant="outline" className="w-full" onClick={removeActive}>
          <Trash2 className="h-4 w-4" /> Xóa đối tượng đã chọn
        </Button>
        <div className="border-t border-border pt-4 space-y-2">
          <Button className="w-full" onClick={() => save(false)}>
            <Save className="h-4 w-4" /> Lưu bản nháp
          </Button>
          <Button className="w-full" variant="outline" onClick={() => save(true)}>
            <Send className="h-4 w-4" /> Gửi cho designer
          </Button>
        </div>
      </aside>
