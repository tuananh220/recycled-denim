...
export default function CartPage() {
  ...
  if (!user) {
    return (
      <div className="container py-24 text-center">
        <h1 className="text-4xl mb-4">Your bag is locked</h1>
        <p className="text-muted-foreground mb-6">Sign in to view items in your bag.</p>
  ...
  if (items.length === 0) {
    return (
      <div className="container py-24 text-center">
        <h1 className="text-4xl mb-4">Your bag is empty</h1>
  ...
  return (
    <div className="container py-12 grid lg:grid-cols-[1fr_360px] gap-12">
            ...
            <li key={i.id} className="py-6 flex gap-4">
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground uppercase tracking-widest mt-1">Size {i.size} · <span className="inline-block w-3 h-3 align-middle rounded-full border" style={{ backgroundColor: i.color }} /></p>
                </div>
              ...
              <button onClick={() => remove(i.id)} aria-label="remove" className="text-muted-foreground hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
            </li>
          ))}
        </ul>
      </div>
      ...
      <aside className="lg:sticky lg:top-24 lg:self-start border border-border p-6 space-y-4">
        <h2 className="text-xs uppercase tracking-widest">Summary</h2>
        <Row label="Subtotal" value={formatCurrency(total)} />
        <Row label="Shipping" value={total > 200 ? 'Free' : formatCurrency(12)} />
        <Row label="Estimated tax" value={formatCurrency(total * 0.08)} />
        <div className="border-t border-border pt-4">
          <Row label="Total" value={formatCurrency(total + (total > 200 ? 0 : 12) + total * 0.08)} bold />
        </div>
        <Button asChild size="lg" className="w-full"><Link href="/checkout">Checkout</Link></Button>
      </aside>
    </div>
  );
}
